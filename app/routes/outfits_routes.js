...
router.get('/outfits/:id', requireToken, (req, res, next) => {
...
// CREATE
// POST /outfits
router.post('/outfits', requireToken, (req, res, next) => {
	// set owner of new outfit to be current user
	req.body.outfit.owner = req.user.id

	Outfit.create(req.body.outfit)
		// respond to succesful `create` with status 201 and JSON of new "outfit"
		.then((outfit) => {
			res.status(201).json({ outfit: outfit.toObject() })
		})
...
router.delete('/outfits/:id', requireToken, (req, res, next) => {
	Outfit.findById(req.params.id)
		.then(handle404)
		.then((outfit) => {
